...
1. Worker Agent sends a POST request to FastAPI.
2. FastAPI saves the request in Postgres (for auditing).
3. FastAPI checks the JSON Policy.
4. FastAPI calls the LLM to get a Risk Score.
5. FastAPI returns a "Go" or "No-Go" to the Worker.

## Existing Applications that Focus on AI Security and Guard
...
- **DeepEval:** In 2026, DeepEval is widely used for "Unit Testing" agents.
